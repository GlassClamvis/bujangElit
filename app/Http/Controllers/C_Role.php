...
class C_Role extends Controller
{
    ...
    public function index(){
        ...
        $Breadcrumb = array(
            ...
            2 => array("is-active" => 1, "link" => "", "label" => "Role List"),
        );
    ...
    public function create()
    {
        ...
        $Breadcrumb = array(
            ...
            2 => array("link" => url("roles"), "label" => "List Role"),
            3 => array("link" => "active", "label" => "Tambah Role"),
        );
    ...
    public function edit($id)
    {
        ...
        $Breadcrumb = array(
            1 => array("link" => url("/"), "label" => "E-PAK"),
            2 => array("link" => url("roles"), "label" => "List Role"),
            3 => array("link" => "active", "label" => "Ubah Role"),
        );
        $role = Role::find($idDecrypt);
        $permission = Permission::get();
        $rolePermissions = DB::table("role_has_permissions")->where("role_has_permissions.role_id",$idDecrypt)
    ...
    public function getRoles(Request $request){
        ...
        foreach ($records as $record) { $number += 1;
           ...
           if(Gate::check('role-list')){
             $button = $button."<a href='#' data-val='$idEncrypt' data-label='$record->name' class='btn btn-info btn-outline btn-circle btn-md m-r-5 roleShow'>
                Show
            </a>";
            }
            if(Gate::check('role-edit')){
                $button = $button." <a href='".route('roles.edit',$idEncrypt)."' data-href='".route('roles.edit',$record->id)."' class='btn btn-primary btn-outline btn-circle btn-md m-r-5'>
                 Edit
            </a>";
            }
 ...
 */
    ...
    public function getRoleShow(Request $request)
    {
        ...
        $role = Role::find($idDecrypt);
        $rolePermissions = Permission::join("role_has_permissions","role_has_permissions.permission_id","=","permissions.id")
            ->where("role_has_permissions.role_id",$idDecrypt)
            ->get(); $data = array();
